...
export const AppProvider = ({ children }) => {
  ...
  // Coaching log operations - UPDATED to support metadata
  const postCoachingLog = async (employeeId, content, category, currentUser, metadata = {}) => {
    try {
      ...
      logsData.push({
        ...
        // Support optional metadata (incidentDates, violationData, etc.)
        ...metadata
      });
  ...
  // Delete coaching log and suppress auto-coaching if it's an auto-generated one
  const deleteCoachingLog = async (logId, currentUser) => {
    try {
      const logsDataFromDB = await db.get('coaching-logs');
      const logsData = Array.isArray(logsDataFromDB) ? logsDataFromDB : [];

      const logToDelete = logsData.find(log => log.id === logId);
      ...
      // If this is an auto-generated coaching (issued by System/Auto-Coaching), suppress it
      if (logToDelete.issuedBy?.includes('System') || logToDelete.issuedBy?.includes('Auto-Coaching')) {
        await suppressAutoCoaching(logToDelete.employeeId, logToDelete.category);
      }

      const updatedLogs = logsData.filter(log => log.id !== logId);
      await db.set('coaching-logs', updatedLogs);
      setCoachingLogs(updatedLogs);
      await addToFeed(`🗑️ Coaching log deleted for ${logToDelete.employeeId}`, 'coaching', currentUser);
      setSuccess('Coaching log deleted! 🗑️');
    } catch (err) {
      setError('Failed to delete coaching log: ' + err.message);
    }
  };

  // Helper function to check if incident should be ignored - IMPROVED
  const shouldIgnoreIncident = async (employeeId, category, incidentDate) => {
    try {
      ...
      // Normalize the incident date to YYYY-MM-DD format
      let dateStr;
      if (incidentDate instanceof Date) {
        dateStr = incidentDate.toISOString().split('T')[0];
      } else if (typeof incidentDate === 'string') {
        // Handle both ISO strings and date strings like "Mon Nov 11 2024"
        const dateObj = new Date(incidentDate);
        if (!isNaN(dateObj.getTime())) {
          dateStr = dateObj.toISOString().split('T')[0];
        } else {
          // If it's already in YYYY-MM-DD format or similar
          dateStr = incidentDate.split('T')[0];
        }
      }

      const isIgnored = ignoredIncidents[ignoredKey].dates.includes(dateStr);

      if (isIgnored) {
        console.log(`✋ Ignoring incident for ${employeeId} (${category}) on ${dateStr}`);
      }
  ...
  // Helper to check if an auto-coaching was manually suppressed
  const isAutoCoachingSuppressed = async (employeeId, category) => {
    try {
      const suppressed = await db.get('suppressed-auto-coaching') || {};
      const key = `${employeeId}_${category}`;

      if (!suppressed[key]) return false;

      // Check if suppression is still valid (within 30 days)
      const suppressedDate = new Date(suppressed[key].date);
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

      const isStillSuppressed = suppressedDate >= thirtyDaysAgo;

      if (isStillSuppressed) {
        console.log(`🚫 Auto-coaching suppressed for ${employeeId} (${category})`);
      }

      return isStillSuppressed;
    } catch (error) {
      console.error('Error checking suppressed coaching:', error);
      return false;
    }
  };

  // Mark an auto-coaching as suppressed (when admin deletes it)
  const suppressAutoCoaching = async (employeeId, category) => {
    try {
      const suppressed = await db.get('suppressed-auto-coaching') || {};
      const key = `${employeeId}_${category}`;
      ...
      await db.set('suppressed-auto-coaching', suppressed);
      console.log(`✅ Suppressed auto-coaching for ${employeeId} (${category}) for 30 days`);
    } catch (error) {
      console.error('Error suppressing auto-coaching:', error);
    }
  };

  // AUTO-COACHING LOGIC - IMPROVED with better logging
  const checkAndTriggerAutoCoaching = async (adminUser) => {
    if (!autoCoachingEnabled || !adminUser) {
      console.log('⏸️ Auto-coaching disabled or no admin user');
      return;
    }

    console.log('🔍 Running auto-coaching check...');
    ...
    console.log(`📊 Checking ${employeeList.length} employees...`);
    ...
    for (const employee of employeeList) {
      ...
      const hasRecentCoaching = (type) => {
        ...
        const recent = coachingLogs.some(log =>
        ...
        if (recent) {
          console.log(`✅ ${employee.name} already has recent ${type} coaching`);
        }
      ...
      if (!hasRecentCoaching('tardiness')) {
        const isSuppressed = await isAutoCoachingSuppressed(employee.employeeId, 'tardiness');

        if (!isSuppressed) {
          const tardinessViolation = await checkTardiness(employee.employeeId);
          if (tardinessViolation?.triggered) {
            console.log(`⚠️ TRIGGERING tardiness coaching for ${employee.name}`);
      ...
      if (!hasRecentCoaching('overbreak')) {
        const isSuppressed = await isAutoCoachingSuppressed(employee.employeeId, 'overbreak');

        if (!isSuppressed) {
          const overbreakViolation = await checkOverbreak(employee.employeeId);
          if (overbreakViolation?.triggered) {
            console.log(`⚠️ TRIGGERING overbreak coaching for ${employee.name}`);
      ...
      if (!hasRecentCoaching('absence')) {
        const isSuppressed = await isAutoCoachingSuppressed(employee.employeeId, 'absence');

        if (!isSuppressed) {
          const absenceViolation = await checkAbsence(employee.employeeId);
          if (absenceViolation?.triggered) {
            console.log(`⚠️ TRIGGERING absence coaching for ${employee.name}`);
    ...
    console.log('✅ Auto-coaching check complete');
  };

  // Check for tardiness violations - IMPROVED
  const checkTardiness = async (employeeId) => {
    ...
    console.log(`📊 ${employeeId} - Found ${incidents.length} tardiness incidents before filtering`);

    // Filter out ignored incidents
    const filteredIncidents = [];
    for (const incident of incidents) {
      const shouldIgnore = await shouldIgnoreIncident(employeeId, 'tardiness', incident.date);
      if (!shouldIgnore) {
        filteredIncidents.push(incident);
      }
    }

    console.log(`📊 ${employeeId} - ${filteredIncidents.length} tardiness incidents after filtering`);

    const filteredCount = filteredIncidents.length;
    const filteredMinutes = filteredIncidents.reduce((sum, inc) => sum + inc.minutesLate, 0);

    if (filteredCount >= 3 || filteredMinutes >= 30) {
      console.log(`🚨 ${employeeId} - TARDINESS VIOLATION: ${filteredCount} incidents, ${filteredMinutes} minutes`);
      return {
        type: 'tardiness',
        employeeId,
        count: filteredCount,
  ...
  // Check for overbreak violations - IMPROVED
  const checkOverbreak = async (employeeId) => {
    const last30Days = new Date();
    last30Days.setDate(last30Days.getDate() - 30);
    ...
    console.log(`📊 ${employeeId} - Found ${incidents.length} overbreak incidents before filtering`);

    // Filter out ignored incidents
    const filteredIncidents = [];
    for (const incident of incidents) {
      const shouldIgnore = await shouldIgnoreIncident(employeeId, 'overbreak', incident.date);
      if (!shouldIgnore) {
        filteredIncidents.push(incident);
      }
    }

    console.log(`📊 ${employeeId} - ${filteredIncidents.length} overbreak incidents after filtering`);

    if (filteredIncidents.length >= 3) {
      console.log(`🚨 ${employeeId} - OVERBREAK VIOLATION: ${filteredIncidents.length} incidents`);
      return {
        type: 'overbreak',
        employeeId,
  ...
  // Check for absence/NCNS - IMPROVED
  const checkAbsence = async (employeeId) => {
    const last30Days = new Date();
    last30Days.setDate(last30Days.getDate() - 30);
    ...
    console.log(`📊 ${employeeId} - Found ${absences.length} absence incidents before filtering`);

    // Filter out ignored incidents
    const filteredIncidents = [];
    for (const incident of absences) {
      const shouldIgnore = await shouldIgnoreIncident(employeeId, 'absence', incident.date);
      if (!shouldIgnore) {
        filteredIncidents.push(incident);
      }
    }

    console.log(`📊 ${employeeId} - ${filteredIncidents.length} absence incidents after filtering`);

    // Recalculate consecutive absences after filtering
    ...
    if (filteredIncidents.length > 0) {
      console.log(`🚨 ${employeeId} - ABSENCE VIOLATION: ${filteredIncidents.length} incidents, ${maxConsecutive} consecutive`);
      return {
        type: 'absence',
        employeeId,
        count: filteredIncidents.length,
        ...
        incidents: filteredIncidents,
        triggered: true,
        severity: maxConsecutive >= 3 ? 'serious' : 'less-serious'
      };
    }
    return null;
  };

  // IMPROVED: Store incident metadata when creating coaching logs
...
Date: ${new Date().toLocaleString()}
    ...
    // Extract incident dates for easy reference and permanent deletion
    const incidentDates = violation.incidents.map(inc => {
      // Convert date string to YYYY-MM-DD format
      const dateObj = new Date(inc.date);
      return dateObj.toISOString().split('T')[0];
    });

    console.log(`💾 Storing coaching log with ${incidentDates.length} incident dates`);

    // Use the updated postCoachingLog with metadata
    await postCoachingLog(
      ...
      currentUser,
      {
        // Store incident metadata for reliable permanent deletion
